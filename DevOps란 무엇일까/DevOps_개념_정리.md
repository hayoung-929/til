...
> **대상**: 광주소프트웨어마이스터고 DevOps 전공 예정 학생  
> **수준**: 입문~중급 (기본 개념 보유, 심화 이해 목표)  
> **전제 지식**: 프로그래밍 기능사, 네트워크 관리사 2급, 리눅스 마스터 2급
...
| 2009 | 첫 번째 DevOpsDays 컨퍼런스 개최 (벨기에 겐트); Flickr "10 deploys per day" 발표 — John Allspaw & Paul Hammond |
| 2010 | "The Phoenix Project" 개념의 전신 등장 |
| 2013 | "The Phoenix Project" (Gene Kim 외) 출간; Docker 0.1 릴리스 — 컨테이너 혁명 시작 |
| 2014 | "The DevOps Handbook" 작업 시작; Kubernetes 구글에서 오픈소스 공개 |
| 2016 | "The DevOps Handbook" (Gene Kim, Jez Humble 외) 출간; DORA(DevOps Research and Assessment) 연구 시작 |
...
**전통적 개발 방식** — 개발팀이 코드를 작성한 후 "벽 너머로" 운영팀에 전달하는 구조:
...
개발팀은 빠른 변경(혁신)을 원하고, 운영팀은 안정성(변화 최소화)을 원하는 갈등이 존재했다.
...
- **Waterfall (폭포수)**: 요구사항 → 설계 → 개발 → 테스트 → 배포 → 유지보수 (각 단계 수개월, 역행 불가)
- **Agile (애자일)**: Sprint 1(계획/개발/테스트) → Sprint 2 → Sprint N → 배포 (2~4주 스프린트, 점진적 개선)
- **DevOps**: Plan → Code → Build → Test → Release → Deploy → Operate → Monitor → (반복) (자동화 + 협업으로 하루에도 여러 번 배포 가능)
...
**DevOps 무한루프** — 개발 단계(Dev)와 운영 단계(Ops)가 끊임없이 순환한다:

PLAN → CODE → BUILD → TEST → RELEASE → DEPLOY → OPERATE → MONITOR → (다시 PLAN으로)

| 단계 구분 | 포함 단계 |
|-----------|----------|
| 개발 단계 (Dev) | Plan, Code, Build, Test |
| 운영 단계 (Ops) | Release, Deploy, Operate, Monitor |
...
**테스트 피라미드** — 아래에서 위로 갈수록 느리고 비용이 높으며, 더 적게 작성한다:

| 테스트 계층 | 특성 | 대표 도구 |
|------------|------|----------|
| 단위 테스트 (Unit) — 하단, 가장 많이 | 빠름, 저렴 | Jest, JUnit, pytest |
| 통합 테스트 (Integration) — 중간 | 중간 속도/비용 | Testcontainers, Postman |
| E2E 테스트 — 상단, 가장 적게 | 느림, 비용 높음 | Selenium, Cypress, Playwright |
...
버전 형식: `MAJOR.MINOR.PATCH` (예: `2.4.1`)

| 필드 | 의미 | 예시 |
|------|------|------|
| PATCH | 버그 수정 (하위 호환) | 2.4.**1** |
| MINOR | 기능 추가 (하위 호환) | 2.**4**.1 |
| MAJOR | 파괴적 변경 (하위 비호환) | **2**.4.1 |

pre-release 표기: `1.0.0-alpha.1`, `1.0.0-beta.2`, `1.0.0-rc.1`

### 2.6 Deploy (배포)

**목적**: 애플리케이션을 대상 환경에 실제로 배포한다.

**배포 환경 흐름**: 코드 → Dev (개발자 테스트) → Staging (QA팀 통합테스트) → Production (실제 사용자 트래픽)
...
| 기둥 | 설명 | 주요 도구 |
|------|------|----------|
| Metrics (메트릭) | 숫자 기반 집계 데이터 | Prometheus, Grafana |
| Logs (로그) | 이벤트 텍스트 기록 | ELK Stack, Loki |
| Traces (분산 추적) | 서비스 간 요청 경로·지연 추적 | Jaeger, Zipkin, OpenTelemetry |
...
**CI 흐름**: 여러 개발자가 각자 git push → CI 서버에서 빌드·테스트 → 성공 시 다음 단계 진행, 실패 시 즉시 개발자에게 알림
...
CI → 빌드 → 테스트 → 스테이징 배포 → **수동 승인 (사람이 버튼 클릭)** → 프로덕션 배포

**Continuous Deployment (지속적 배포)**:  
CI → 빌드 → 테스트 → 스테이징 배포 → **자동 승인 (모든 게 통과하면 자동)** → 프로덕션 배포
...
| 계층 | 가상 머신 (VM) | 컨테이너 |
|------|---------------|---------|
| 애플리케이션 | App A / App B (별도) | App A / App B (별도) |
| 격리 계층 | Guest OS (각 VM마다) | Container Runtime (각 컨테이너) |
| 가상화 엔진 | Hypervisor | Docker Engine |
| 기반 OS | Host OS | Host OS |
...
Control Plane (마스터) 구성 요소:

| 구성 요소 | 역할 |
|----------|------|
| API Server | 진입점, 모든 요청 처리 |
| etcd | 클러스터 상태 저장소 |
| Controller Manager | 원하는 상태 지속 유지 |
| Scheduler | 파드를 노드에 배치 |

Worker Node 구성 요소 (각 노드마다):

| 구성 요소 | 역할 |
|----------|------|
| kubelet | 노드의 에이전트, 파드 실행 관리 |
| kube-proxy | 네트워크 프록시, 서비스 라우팅 |
| Pod(s) | 실제 컨테이너 실행 단위 |

Control Plane이 여러 Worker Node(Node 1, Node 2, Node 3 등)를 관리하며, 각 노드에는 kubelet, kube-proxy, 파드들이 실행된다.
...
1. 개발자가 git push (원하는 상태 정의)
2. GitOps 에이전트(ArgoCD / Flux)가 Git 저장소를 자동 감지 (Pull 방식)
3. 클러스터 실제 상태와 Git 정의 상태 비교 (Desired vs Actual)
4. 차이가 있으면 자동 동기화하여 실제 운영 상태 갱신
...
**Shift Left Security** — 파이프라인 초기(왼쪽)에 보안을 적용할수록 비용이 낮고 수정이 빠르다:
...
**DevOps 도구 맵** — 라이프사이클 단계별 대표 도구:

| 단계 | 대표 도구 |
|------|----------|
| Plan | Jira, GitHub Issues, GitLab Issues |
| Code | Git, GitHub, GitLab |
| Build | Maven, Gradle, Docker |
| Test | Jest, JUnit, Selenium |
| Release | GitHub Actions, Jenkins |
| Deploy | ArgoCD, Flux, Helm |
| Operate | Kubernetes, Ansible, Terraform |
...
|------|---------|---------------|
| 커스터마이징 | ✅ 완전한 커스터마이징 | ✅ GitHub와 원활한 통합 |
| 생태계 | ✅ 수천 개 플러그인 | ✅ 마켓플레이스 (Actions) |
| 운영 부담 | ✅ 셀프 호스팅 (보안 환경) / ❌ 설치·유지보수 부담 | ✅ 관리 부담 없음 (클라우드) |
| 문법 | ❌ Groovy DSL 학습 필요 | ✅ 간단한 YAML 문법 |
| UI | ❌ 오래된 UI | ✅ 무료 공개 저장소 |
| 의존성 | — | ❌ GitHub 의존성 / ❌ 셀프 호스팅 러너 설정 필요 |
...
|------|-----------|--------------|
| 검색 | ✅ 강력한 전문 검색 | ❌ 검색 기능 상대적 제한 |
| 기능 | ✅ 풍부한 기능 | — |
| 생태계 | ✅ 성숙한 생태계 | ❌ ELK보다 젊은 생태계 |
| 비용 | ❌ 높은 리소스 사용 / ❌ 높은 비용 | ✅ 저비용 (인덱싱 최소화) |
| 운영 | ❌ 복잡한 운영 | ✅ Grafana와 완벽 통합 |
| 쿼리 | — | ✅ Prometheus 유사 쿼리 (LogQL) |
| K8s 통합 | — | ✅ K8s 네이티브 |
...
1. **팀 규모와 기술 수준**
   - 소규모 팀 → Ansible + GitHub Actions (단순)
   - 대규모 팀 → Terraform + Jenkins + ArgoCD (강력)
...
| 요소 | 설명 |
|------|------|
| 협업 (Collaboration) | 개발 + 운영 공동책임, 팀 간 투명한 소통 |
| 자동화 (Automation) | 반복 작업 제거 |
| 피드백 루프 (Fast Feedback) | 실시간 메트릭 공유, 빠른 실패·빠른 수정 |
| 지속적 학습 (Continuous Learning) | 실험 장려, 실패에서 학습 |

네 요소는 서로 연결되어 상호 강화한다. 협업과 자동화가 빠른 피드백을 가능하게 하고, 이를 통해 지속적으로 학습하며 다시 협업 문화가 강화된다.
...
**DevOps와 SRE의 관계**: DevOps는 "무엇을(철학/문화/방법론)"에 해당하고, SRE는 그것을 구체적으로 구현하는 "어떻게(Google의 DevOps 구현체)"에 해당한다.
...
**Platform Engineering 계층 구조**:

| 계층 | 역할 |
|------|------|
| 개발 팀 (소비자) | 코드 작성에만 집중, 인프라 신경 안 써도 됨 |
| Internal Developer Platform (IDP) | 배포 버튼 클릭, 환경 자동 생성, 모니터링 자동화 제공 |
| Platform Team (플랫폼 팀) | IDP 구축 및 관리 |

개발 팀은 셀프 서비스 방식으로 IDP를 사용하고, 플랫폼 팀은 IDP를 관리하는 구조다.
...
**모놀리식 아키텍처** — 모든 기능이 하나의 애플리케이션 안에 통합되고, 단일 DB를 공유한다:
...
**마이크로서비스 아키텍처** — 각 기능이 독립적인 서비스로 분리되며, 서비스별로 독립 DB를 사용한다. 서비스 간 통신은 API Gateway 또는 Message Bus를 통한다:
...
모든 서비스는 API Gateway / Message Bus를 통해 상호 통신한다.
...
| 항목 | 가변 인프라 (Mutable) | 불변 인프라 (Immutable) |
|------|----------------------|------------------------|
| 변경 방법 | SSH 접속 → apt upgrade → 설정 파일 변경 → 서비스 재시작 | 새 이미지로 빌드 → 테스트 → 트래픽 전환 → 구버전 삭제 |
| 결과 | "눈송이 서버" — 예측 불가능, 재현 불가능, 드리프트 발생 | 완전한 재현성 ✅, 드리프트 없음 ✅, 롤백 용이 ✅ |
...
Blue/Green 배포는 두 개의 동일한 환경을 운영하며 트래픽을 전환하는 방식이다.

**현재 상태** (Blue가 프로덕션):
- Blue (v1.0) ← 사용자 트래픽 수신 중 (프로덕션)
- Green (v2.0) ← 신버전 배포 완료, 대기 중

**전환 후 상태** (Green이 프로덕션):
- Blue (v1.0) ← 대기 중 (롤백 준비)
- Green (v2.0) ← 사용자 트래픽 수신 중 (프로덕션)
...
카나리 배포는 신버전을 소수 트래픽부터 단계적으로 확대하는 방식이다.

1단계 (5% 카나리): 95% 트래픽 → v1.0 (기존), 5% 트래픽 → v2.0 (카나리) → 모니터링 (에러율, 응답시간)  
2단계 (25% 카나리): 75% 트래픽 → v1.0, 25% 트래픽 → v2.0 → 계속 모니터링  
3단계 (완전 전환): 100% 트래픽 → v2.0

문제 발생 시 언제든 트래픽을 v1.0으로 돌려 롤백한다.

#### Rolling Update (롤링 업데이트)

롤링 업데이트는 파드를 하나씩 순차적으로 교체하는 방식이다.

| 단계 | 파드 상태 |
|------|----------|
| 초기 상태 | v1.0, v1.0, v1.0, v1.0 |
| Step 1 | v2.0, v1.0, v1.0, v1.0 |
| Step 2 | v2.0, v2.0, v1.0, v1.0 |
| Step 3 | v2.0, v2.0, v2.0, v1.0 |
| 완료 | v2.0, v2.0, v2.0, v2.0 |
...
1. **정상 상태(Steady State) 정의** — 예: "초당 1000 요청, 에러율 0.1% 이하"
2. **가설 수립** — 예: "서버 1대가 다운돼도 정상 상태를 유지할 것"
3. **장애 주입 (실험)** — 서버 종료, 네트워크 지연, CPU 과부하 등
4. **관찰 및 검증** — 정상 상태가 유지되는지 모니터링
5. **결과 학습 및 시스템 개선** — 취약점 발견 → 수정 → 재실험
...
| 분류 | 메트릭 | 의미 |
|------|--------|------|
| 속도 (Velocity) | 배포 빈도 (Deployment Frequency) | 얼마나 자주 배포하나? |
| 속도 (Velocity) | 변경 리드 타임 (Lead Time for Changes) | 커밋부터 프로덕션까지 걸리는 시간 |
| 안정성 (Stability) | 변경 실패율 (Change Failure Rate) | 배포 중 실패 비율 = 실패 배포 / 전체 |
| 안정성 (Stability) | 서비스 복구 시간 (MTTR) | 장애 발생부터 복구까지 시간 |
...
**SLA / SLO / SLI 관계** — 계층 구조로 이해한다:

| 개념 | 의미 | 예시 |
|------|------|------|
| SLI (Service Level Indicator) | 실제로 측정되는 숫자 지표 | 지난 30일 가용성 = 99.95% |
| SLO (Service Level Objective) | 팀이 달성하려는 내부 목표 | 30일 가용성 ≥ 99.9% |
| SLA (Service Level Agreement) | 고객과의 법적 약속·계약 | 99.9% 가용성 미달 시 서비스 크레딧 지급 |

SLA가 가장 바깥 계층이고, 그 안에 SLO(내부 목표, SLA보다 엄격), 그 안에 SLI(실제 측정값)가 있다.

SLO를 SLA보다 엄격하게 설정하는 이유:
- SLI 99.95% > SLO 99.9% > SLA 99.5%
- SLO 달성 실패 → 경고 (내부 조치)
- SLA 달성 실패 → 고객 환불 (법적 의무)
...
**완전한 DevOps 파이프라인** — 개발자 PC에서 운영 모니터링까지의 전체 흐름:
...
8. 컨테이너 이미지 빌드: `docker build -t myapp:$GIT_SHA .`
9. 이미지 푸시: `docker push ghcr.io/myorg/myapp:$GIT_SHA`
10. 스테이징 배포 (자동): ArgoCD sync (staging 네임스페이스)
11. 통합 테스트 / E2E 테스트 (스테이징에서 실행): Playwright, k6 성능 테스트
...
13. Canary 배포 시작 (5% 트래픽): ArgoCD + Argo Rollouts
...
1. **감지 (Detection)**: Prometheus Alert → Alertmanager → PagerDuty
2. **인지 (Acknowledge)**: 온콜 담당자 PagerDuty 확인 (목표: 5분 이내)
3. **분류 (Triage)**:
...
4. **초기 대응 (Mitigation)**:
   ...
   - 롤백 판단: 최근 배포가 원인이면 즉시 롤백, 아니면 원인 파악 계속
...
6. **서비스 복구 확인**: 모니터링 정상화 → 이해관계자 알림
7. **근본 원인 분석 (RCA)**: 72시간 내 Blameless Postmortem 작성
...
**IaC 기반 인프라 프로비저닝 워크플로우** — "신규 마이크로서비스를 위한 EKS 클러스터 필요" 요구사항을 예시로:

1. **설계**: 네트워크(VPC, 서브넷, 보안 그룹), 컴퓨팅(EKS 클러스터, 노드 그룹), 스토리지(RDS, S3) 설계
2. **Terraform 코드 작성**: `modules/vpc/main.tf`, `modules/eks/main.tf`, `environments/prod/main.tf`
3. **코드 리뷰 (PR)**: 팀원 검토 → 승인
4. **terraform plan (CI에서 자동 실행)**: 변경사항 미리 확인 (PR 코멘트로 표시)
5. **PR 머지 → terraform apply (자동 or 수동)**: 실제 AWS 리소스 생성
6. **Ansible로 초기 설정**: K8s 애드온 설치 (metrics-server, ingress-nginx)
7. **ArgoCD 설치 및 Git 저장소 연동**: GitOps 자동화 활성화
8. **모니터링 스택 배포**: kube-prometheus-stack (Helm)
...
**DevOps 엔지니어 기술 맵** — 영역별 핵심 기술:

| 영역 | 기술 | 비고 |
|------|------|------|
| **기초 (Foundation)** | Linux | 이미 보유 ✅ |
| | Networking | 이미 보유 ✅ |
| | 프로그래밍 (Python, Bash, Go) | — |
| | Git | 이미 보유 ✅ |
| **컨테이너 & 오케스트레이션** | Docker | 필수 |
| | Kubernetes | 필수 |
| | Helm | K8s 패키지 관리 |
| **CI/CD** | GitHub Actions | 추천 시작점 |
| | Jenkins | 선택 |
| | ArgoCD | GitOps |
| **인프라 관리** | Terraform | 필수 |
| | Ansible | 추천 |
| | Vault | 시크릿 관리 |
| **클라우드** | AWS | 추천 시작점, 점유율 1위 |
| | Azure | 선택 |
| | GCP | 선택 |
...
**Phase 1 — 기초 강화 (1~2개월)**
- 리눅스 심화 (파일시스템, 프로세스, 네트워크 설정, 쉘 스크립팅, systemd, cron)
- Git / GitHub 실습 (Git Flow 브랜치 전략, GitHub Actions 기초)
- Python 기초 (자동화 스크립팅, 파일 처리, requests, boto3)

**Phase 2 — 컨테이너 (2~3개월)**
- Docker 기초 → 심화 (Dockerfile 작성, Docker Compose, 멀티스테이지 빌드)
- Docker로 미니 프로젝트 (간단한 웹앱 컨테이너화)

**Phase 3 — CI/CD 파이프라인 (2개월)**
- GitHub Actions (기본 워크플로우, 시크릿 관리, Matrix Build)
- Jenkins 선택 (Declarative Pipeline)
...
- AWS 핵심 서비스 (EC2, VPC, S3, RDS, IAM, EKS, ECR)
- Terraform 기초 → 심화 (Module 작성, Remote State, Workspace)
...
- 전체 스택 통합 프로젝트 (GitHub → CI/CD → Docker → K8s → 모니터링)
...
| 계층 | 내용 |
|------|------|
| **문화 (Culture)** | 협업 · 투명성 · 공동책임 · 심리적 안전감 · 지속적 학습 |
| **라이프사이클 (∞ 무한루프)** | Plan → Code → Build → Test → Release → Deploy → Operate → Monitor |
| **실천방법 (Practices)** | CI/CD, IaC, 컨테이너, GitOps |
| **도구 (Tools)** | Git · Docker · K8s · Terraform · Ansible · Prometheus · ELK |
| **측정 (Measurement)** | DORA 메트릭 · SLA/SLO/SLI · Error Budget |
